...
layui.use(['form','jquery',"layer"],function() {
    ...
    if(window.sessionStorage.getItem('userFace') &&  $(".userAvatar").length > 0){
    	 $("#userFace").attr("src",ftpip+window.sessionStorage.getItem('userFace'));
         $(".userAvatar").attr('src', ftpip+window.sessionStorage.getItem('userFace'));
    }else{
        $(".userAvatar").attr("src",webstaticname+"/resouce/html/layuicms2.0/images/face.jpg");//默认头像
    }
    ...
    function showNotice(){
        layer.open({
            ...
            btn: ['确定'],
            moveType: 1,
            content: '<div style="padding:15px 20px; text-align:justify; line-height: 22px; text-indent:2em;border-bottom:1px solid #e2e2e2;"><p class="layui-red">马上要元旦了，祝大家元旦快乐</p></div>',
...
function lockPage(){
   shuopin=layer.open({
            ...
            content : '<div class="admin-header-lock" id="lock-box">'+
                            '<div class="admin-header-lock-img"><img src="'+ftpip+headImg+'" class="userAvatar"/></div>'+
    ...
    $("body").on("click","#unlock",function(){
        if($(this).siblings(".admin-header-lock-input").val() == ''){
        	...
        	$.post(webname+"/backsystem/index/login",param,function(d){
        		if(qjty(d)){
	        		if(d.code>0){
    ...
    gongneng=layer.open({
            ...
            success : function(index, layero){
                ...
                form.on("submit(settingSuccess)",function(data){
                    ...
                    $.post(webname+"/backsystem/system/isSingle",{isSingle:isSingle,uid:uid},function(d){
                    	if(qjty(d)){
                    		layer.msg(d.msg);
                    	}
