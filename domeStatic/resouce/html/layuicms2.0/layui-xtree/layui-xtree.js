...
layuiXtree.prototype.dataBind = function (d) {
    ...
    if (d.length > 0) {
        for (i in d) {
            ...
            _this._domStr += '<input type="checkbox" class="layui-xtree-checkbox" ' + xtree_isend + xtree_ischecked + xtree_isdisabled +' data="'+d[i].jibie+'" value="' + d[i].value + '" title="' + d[i].title + '" lay-skin="primary" lay-filter="xtreeck' + _this._containerid + '">';
...
layuiXtree.prototype.GetChecked = function () {
    ...
    for (var i = 0; i < cks.length; i++) {
    	console.log(cks[i].getAttribute("data"));
        if (cks[i].checked && cks[i].getAttribute("data")!=null/* && cks[i].getAttribute('data-xend') == '1'*/) {
